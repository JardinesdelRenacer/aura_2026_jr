...
import { emptyObituary, type Obituary, type RoomKeys, type MediaItem } from "@/app/proyectar/page";
            ...
            <div className={screenClassName}>
                {isVertical ? (
                    (() => {
                        ...
                        const obituary = roomKey ? (obituaries[roomKey] ?? emptyObituary()) : null;
                        return (
                    ...
                    <div className="h-full w-full bg-linear-to-br from-white/60 via-blue-50/50 to-white/40 p-1.5 backdrop-blur-2xl sm:p-2">
                        <div className="grid h-full w-full gap-1.5 sm:gap-2" style={{ gridTemplateColumns: `repeat(${splitColumns}, minmax(0, 1fr))`, gridTemplateRows: `repeat(${splitRows}, minmax(0, 1fr))` }}>
                            <div className="relative h-full min-h-0 w-full min-w-0 overflow-hidden rounded-xl border border-white/80 bg-[url('/imagenes/35.png')] bg-cover bg-center shadow-xl sm:rounded-2xl">
                            ...
                            {roomsToShow
                                ...
                                .map(({ roomKey, ob, isActive }) => {
                                    return (
                                        <div key={roomKey} className="relative flex h-full min-h-0 w-full min-w-0 flex-col items-center justify-start overflow-hidden rounded-lg border border-white/30 bg-[url('/imagenes/35.png')] bg-size-[100%_100%] bg-no-repeat text-center shadow-lg sm:rounded-xl">
                                                    ...
                                                    <div className="flex flex-col grow w-full justify-center items-center z-10 overflow-hidden">
                                                        ...
                                                        {(ob.timeStart || ob.timeEnd || ob.endDate || ob.endTime) && (
                                                            <div className="bg-white/30 border border-black/10 rounded-lg p-1 sm:p-2 backdrop-blur-md shadow-sm flex flex-col justify-center overflow-hidden">
                                                                    <p className="text-black/80 text-[0.4rem] uppercase tracking-widest mb-0.5 font-bold [text-shadow:0_1px_2px_rgb(255_255_255)] truncate">Inicio</p>
                                                                    ...
                                                                    {(ob.endDate || ob.endTime) && <p className="mt-0.5 truncate text-[0.4rem] font-semibold text-black/75">Finaliza: {ob.endDate && formatDate(ob.endDate)} {ob.endTime && formatTime(ob.endTime)}</p>}
                                                                </div>
                                                            )}
                                                            {ob.cemetery && (
                                                                <div className="bg-white/30 border border-black/10 rounded-lg p-1 sm:p-2 backdrop-blur-md shadow-sm flex flex-col justify-center overflow-hidden">
                    ...
                    <div className="grid h-full w-full gap-4 border border-white/80 bg-linear-to-br from-white/60 via-blue-50/50 to-white/40 p-4 shadow-[inset_0_0_20px_rgba(255,255,255,0.9),0_8px_32px_rgba(0,0,0,0.1)] backdrop-blur-2xl" style={{ gridTemplateColumns: `repeat(${obituaryColumns}, minmax(0, 1fr))`, gridTemplateRows: `repeat(${obituaryRows}, minmax(0, 1fr))` }}>
                        {roomsToShow
                            .map((roomKey) => {
                                    const ob = obituaries[roomKey] ?? emptyObituary();
                            ...
                            .sort((a, b) => Number(b.isActive) - Number(a.isActive))
                            .map(({ roomKey, ob, isActive }) => (
                                <div key={roomKey} className="bg-[url('/imagenes/35.png')] bg-size-[100%_100%] bg-no-repeat border border-white/20 rounded-2xl p-6 flex flex-col justify-start items-center text-center shadow-xl relative overflow-hidden">
                                            ...
                                            <div className="mt-auto grid grid-cols-2 gap-2 w-full">
                                                {(ob.timeStart || ob.timeEnd || ob.endDate || ob.endTime) && (
                                                    <div className="bg-white/30 border border-black/10 rounded-xl p-2 backdrop-blur-md shadow-lg flex flex-col justify-center overflow-hidden">
                                                        <p className="text-black/80 text-[0.5rem] sm:text-[0.6rem] uppercase tracking-widest mb-0.5 font-bold [text-shadow:0_1px_2px_rgb(255_255_255)] truncate">Inicio</p>
                                                        <p className="text-xs sm:text-sm font-bold text-black [text-shadow:0_1px_3px_rgb(255_255_255)] truncate">
                                                            {ob.timeStart && formatTime(ob.timeStart)} {ob.timeStart && ob.timeEnd && "-"} {ob.timeEnd && formatTime(ob.timeEnd)}
                                                        </p>
                                                        {(ob.endDate || ob.endTime) && <p className="mt-1 truncate text-[0.55rem] font-semibold text-black/75">Finaliza: {ob.endDate && formatDate(ob.endDate)} {ob.endTime && formatTime(ob.endTime)}</p>}
                                                    </div>
                                                )}
                                                {ob.cemetery && (
                                                    <div className="bg-white/30 border border-black/10 rounded-xl p-2 backdrop-blur-md shadow-lg flex flex-col justify-center overflow-hidden">
