...
export default function ObituarioVertical({ obituary, formatDate, formatTime }: Props) {
    ...
    return (
        <div className="relative flex h-full w-full flex-col items-center justify-start overflow-hidden rounded-[1.6rem] bg-[url('/imagenes/35.png')] bg-cover bg-center bg-no-repeat p-4 text-center text-black sm:p-6">
            ...
            <h2 className="z-10 mb-2 w-3/4 border-b border-black/20 pb-1.5 text-sm font-bold uppercase tracking-[0.2em] [text-shadow:0_1px_3px_rgb(255_255_255)] sm:text-lg">
                En memoria de
            </h2>

            {isActive ? (
                <div className="z-10 flex min-h-0 w-full flex-col items-center">
                    <h3 className="w-full truncate px-2 text-3xl font-extrabold [text-shadow:0_1px_3px_rgb(255_255_255)] sm:text-5xl">
                        {obituary.name}
                    </h3>
                    <h3 className="mb-2 w-full truncate px-2 text-2xl font-bold text-black/90 [text-shadow:0_1px_3px_rgb(255_255_255)] sm:text-4xl">
                        {obituary.surname}
                    </h3>

                    {(obituary.dob || obituary.dod) && (
                        <div className="mb-2 flex max-w-full items-center gap-2 overflow-hidden whitespace-nowrap rounded-full border border-black/10 bg-white/50 px-4 py-1.5 text-sm font-medium shadow-lg backdrop-blur-sm sm:text-lg">
