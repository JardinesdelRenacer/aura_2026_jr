...
export default function UploadMedia({
    ...
    const reactId = useId();
    // Cada sala tendrá su propio ID para la multimedia
    const inputId = `file-upload-${room ?? "general"}-${reactId.replace(/:/g, "" )}`;
    ...
    const handleChange =  async(
        ...
        const archivosConRoom = nuevosArchivos.map((file) => {
            const fileWithRoom = file as File & {
                room?: string;
            };
        ...
        <label htmlFor={inputId} className={`cursor-pointer inline-flex items-center justify-center px-8 py-4 font-bold text-white transition-all duration-300 rounded-xl backdrop-blur-md w-full sm:w-auto text-center overflow-hidden relative group ${
            isUploading
                ...
                : showSuccess
                    ...
                    : "bg-gradient-to-r from-green-500/40 to-emerald-600/40 border-green-400/50 shadow-[0_8px_32px_0_rgba(34,197,94,0.3)] hover:bg-green-500/60 hover:shadow-[0_8px_32px_0_rgba(34,197,94,0.5)] hover:-translate-y-1"
            }`} >
                <div className="absolute inset-0 w-full h-full bg-gradient-to-t from-transparent via-white/5 to-white/20" />
                        ...
                        <>
                            ...
                            >
                                ...
                                <path 
                                    ...
                                    fill="currentColor"
                                    d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 0 14 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"
                                />
                            </svg>
